...
import {addCommentService} from "@/api/topiccommentApi.js";
...
const creatcomment=async ()=>{
  if (deltaToText(content.value).length > 2000) {
    ElMessage.warning('评论字数已经超出最大限制2000，请缩减评论内容！')
    return
  }
  await addCommentService(props.tid,props.quote ? props.quote.id : -1,JSON.stringify(content.value))
    ...
    <el-drawer :model-value="show"
      ...
      <div>
        ...
        <div style="margin-top: 10px;display: flex">
          ...
          <el-button type="success" @click="creatcomment" plain>发表评论</el-button>
        </div>
      </div>
    </el-drawer>
  </div>
